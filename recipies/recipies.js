...
let recipeContainer = document.querySelector(".recipe-layout");
...
function search() {
	let searchedInput = document.querySelector("#search").value;

	recipeContainer.innerHTML = "";

	let filteredRecipes = recipes.filter(function(recipe) {
		return (
			recipe.name.toLowerCase().includes(searchedInput.toLowerCase()) ||
			recipe.description.toLowerCase().includes(searchedInput.toLowerCase()) ||
			recipe.tags.find((tag) => tag.toLowerCase().includes(searchedInput.toLowerCase()))
		)
	})
...
function recipeTemplate(recipe) {
	return `<div class="recipe">
				<img src="images/${recipe.image}" alt="${recipe.name} picture">
				<div class="recipe-content">
					${tagTemplate(recipe.tags)}
					<h2>${recipe.name}</h2>
					<div class="rating-container">${ratingTemplate(recipe.rating)}</div>
